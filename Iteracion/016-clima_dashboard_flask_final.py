#!/usr/bin/env python3
# ============================================================
# 015-clima_dashboard_flask.py (FINAL)
# ------------------------------------------------------------
...
def build_hourly_grouped(data: dict) -> List[Dict[str, Any]]:
    ...
    Devuelve lista de bloques:
      [
        {"day":"YYYY-MM-DD", "rows":[...48h del día...]}
      ]
    """
    hourly = data.get("hourly", {}) or {}
...
def update_all_once():
    """
    Actualiza todas las ciudades una vez (usado por botón "Actualizar ahora").
    Evita correr en paralelo con UPDATE_RUNNING_LOCK.
    """
    if not UPDATE_RUNNING_LOCK.acquire(blocking=False):
        return  # ya hay una actualización corriendo
...
def updater_loop():
    """
    Loop principal:
      - Si UPDATE_NOW_EVENT está activo => actualiza inmediatamente
      - Si no => espera interval_seconds
    """
    while not STOP_EVENT.is_set():
        # 1) Si pidieron actualización manual
        if UPDATE_NOW_EVENT.is_set():
            UPDATE_NOW_EVENT.clear()
            update_all_once()

        # 2) Actualización periódica normal
        with CONFIG_LOCK:
            cfg = load_config()
        settings = cfg.get("settings", {}) or {}
        cities = get_cities(cfg)
        ...
        # Espera (pero se puede interrumpir por update_now o stop)
...
<body>
  <div class="topbar">
    <div>
      <h1>🌦️ Clima Analyzer</h1>
      <div class="meta">
        Máx {{ max_cities }} ciudades · Worker cada <b>{{ interval }}</b>s · UI refresh {{ ui_refresh }}s ·
        Retención JSON: {{ retention }} · Horas: {{ hours }} · TZ: {{ tz }}
      </div>
    </div>

    {% if msg %}
      <div class="pill"><span class="dot"></span>{{ msg }}</div>
    {% endif %}
  </div>
...
<head>
  ...
  <style>
    ...
    a{ color:#2563eb; text-decoration:none; font-weight:800; margin-left:10px; }
  </style>
</head>
<body>
  <div class="card">
...
def update_now():
    # Lanza actualización en background (no bloquea el request)
    UPDATE_NOW_EVENT.set()

    # Opcional: disparar un thread inmediato para que no espere al loop a reaccionar
...
def remove_city(city_id: str):
    ...
    # borrar datos para no acumular basura
    try:
